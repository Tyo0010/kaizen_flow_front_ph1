...
import { API_BASE_URL, getAuthHeaders, handleApiResponse, getSessionStatus, getSessionProcessedData, requestPresignedUrls, uploadToS3, confirmUpload } from '../utils/api'
...
function MainPage() {
  ...
  const convertApiDataToUI = (apiData: ProcessedDataItem[]): { generalInformation: GeneralInformation; jobCargo: JobCargo } => {
    ...
    console.log('convertApiDataToUI - Generated general information:', JSON.stringify(generalInformation, null, 2))
  ...
  const handlePreview = async () => {
    ...
    if (!isProcessingComplete || !uploadSessionId) {
      setMessage('Please upload files first and wait for processing to complete before previewing.')
      return
    }
    ...
    try {
      ...
      if (error.message?.includes('No completed documents found')) {
        setMessage('Processing is still in progress. Please wait for documents to complete processing before previewing.')
  ...
  const fetchOutputFormats = async () => {
    ...
    try {
      console.log('Fetching output formats from:', `${API_BASE_URL}/documents/formats`)
      ...
      setMessage('Failed to load output formats. Please check your connection and try again.')
    } finally {
  ...
  const checkSessionStatus = async (sessionId: string): Promise<SessionStatusResponse | null> => {
    try {
      ...
      if (error.message?.includes('404') || error.message?.includes('not found')) {
        console.log('Session not found - may still be initializing or no documents match this session ID')
        return null
      }
      ...
      if (error.message?.includes('500') || error.message?.includes('Internal server error')) {
        ...
        setMessage('Server error while checking status. Will continue trying...')
        return null
      }
  ...
  const manualCheckStatus = async () => {
    ...
    if (status) {
      ...
      const completedDocs = status.documents?.filter((doc: any) => doc.processing_status === 'completed') || []
      ...
      if (status.session_status === 'completed' || status.session_status === 'partially_completed') {
  ...
  const startPollingSession = (sessionId: string) => {
    ...
    const pollFunction = async () => {
      attemptCount++
      console.log(`Polling session status for: ${sessionId} (attempt ${attemptCount}/${maxAttempts})`)
      ...
      if (status) {
        ...
        switch (status.session_status) {
          case 'uploaded':
            setMessage('Files uploaded successfully! Processing will begin shortly...')
            break
          case 'processing':
            setMessage(`Processing ${status.processing_files} of ${status.total_files} files...`)
            break
          case 'completed':
            if (hasCompletedDocsReady) {
              setMessage(`All ${status.total_files} files processed successfully! You can now preview the extracted data.`)
              setIsProcessingComplete(true)
            } else {
              setMessage(`Processing completed but no documents are ready yet. This may indicate an issue with document processing.`)
              setIsProcessingComplete(false)
            }
            // Always stop polling when session status is 'completed'
            if (pollInterval) clearInterval(pollInterval)
            setPollingSession(null)
            setUploading(false) // Clear uploading state to enable upload button
            console.log('Processing completed, stopping polling')
            break
          case 'partially_completed':
            if (hasCompletedDocsReady) {
              setMessage(`${status.completed_files} of ${status.total_files} files processed successfully. ${status.failed_files} failed. Preview available for completed files.`)
              setIsProcessingComplete(true)
            } else {
              setMessage(`${status.completed_files} of ${status.total_files} files processed but documents are not ready yet. This may indicate an issue.`)
              setIsProcessingComplete(false)
            }
            // Always stop polling when session status is 'partially_completed'
            if (pollInterval) clearInterval(pollInterval)
            setPollingSession(null)
            setUploading(false) // Clear uploading state to enable upload button
            console.log('Processing partially completed, stopping polling')
            break
          case 'failed':
        ...
        console.log(`No status received, session may not exist yet (attempt ${attemptCount}/${maxAttempts})`)
        ...
        if (attemptCount >= maxAttempts) {
          ...
          setPollingSession(null)
          setUploading(false) // Clear uploading state to enable upload button
          setMessage('Unable to check processing status. Please check your documents page or contact support.')
        } else {
          // Update message to show we're still trying
          setMessage(`Waiting for processing to begin... (${attemptCount}/${maxAttempts} checks)`)
        }
      }
    }

    // Start polling after a 5-second delay to give the backend time to create the session
    setTimeout(() => {
      setMessage('Starting status checks...')
      // Do the first check immediately after the delay
      pollFunction()
      
      // Then set up the interval for subsequent checks
      pollInterval = setInterval(pollFunction, 15000) // Poll every 15 seconds
  ...
  const handleFiles = (files: FileList) => {
    ...
    Array.from(files).forEach(file => {
      console.log(`File Upload Attempt: Name: ${file.name}, Type: '${file.type}'`);
    });
    ...
    if (validFiles.length !== files.length) {
      alert('Invalid file type detected. Please upload only PDF or supported Excel files (.xls, .xlsx, .xlsm).');
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const presignedResponse: PresignedUrlResponse = await requestPresignedUrls(fileInfos, outputFormat)
      ...
      const uploadPromises = selectedFiles.map(async (file, index) => {
        ...
        return uploadToS3(file, presignedUrl.upload_url, presignedUrl.content_type || file.type)
      })
      ...
      const confirmationFiles: UploadConfirmFile[] = presigned_urls.map((url: PresignedUrlInfo, index: number) => ({
        s3_key: url.s3_key,
        filename: url.filename,
        size: selectedFiles[index].size
      }))
      
      const confirmResponse: UploadConfirmResponse = await confirmUpload(upload_session_id, outputFormat, confirmationFiles)
      ...
      setMessage(`Upload successful! Files are being processed. Session ID: ${upload_session_id}`)
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-4xl font-bold text-gray-800 mb-8">Submit Your Files</h1>

      {/* Message display */}
      {message && (
        <Alert variant={message.includes('Error') || message.includes('Failed') ? 'destructive' : 'default'} className="w-full max-w-lg">
      ...
      <div className="w-full max-w-lg">
        <Select value={outputFormat} onValueChange={setOutputFormat}>
          <SelectTrigger className="w-full h-12">
            <SelectValue placeholder={loadingFormats ? "Loading formats..." : "Select Output Format"} />
          </SelectTrigger>
          <SelectContent>
            {loadingFormats ? (
              <SelectItem value="loading" disabled>Loading formats...</SelectItem>
            ) : outputFormats.length === 0 ? (
              <SelectItem value="no-formats" disabled>No formats available</SelectItem>
            ) : (
      ...
      <p className="text-gray-600 text-left max-w-lg">
        Choose the desired output format for your files. This will determine the type of file you receive after conversion.
      </p>

      {/* File upload area */}
      <Card
        className={`w-full max-w-4xl border-2 border-dashed p-16 text-center transition-colors cursor-pointer ${dragActive ? 'border-blue-500 bg-blue-50' : 'border-blue-300'
          }`}
      ...
      >
        <CardContent className="flex flex-col items-center justify-center gap-2 p-0">
          <h2 className="text-2xl font-semibold text-gray-700">Drag and drop PDF or Excel files here</h2>
          ...
          <p className="text-sm text-gray-400 mt-2">PDF and Excel files are accepted • Minimum 1 file required</p>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {selectedFiles.map((file, index) => {
                ...
                return (
                  <div key={index} className="group flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-100 transition-all duration-200">
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center shadow-sm ${isPDF ? 'bg-gradient-to-br from-red-100 to-red-200' :
                        isExcel ? 'bg-gradient-to-br from-green-100 to-green-200' :
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-gray-900 font-medium truncate max-w-[500px] mb-1" title={file.name}>
                          {file.name}
                        </p>
                        <div className="flex items-center gap-4 text-sm text-gray-500">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {isPDF ? 'PDF Document' : isExcel ? 'Excel Spreadsheet' : 'Document'}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-medium text-blue-800">
                    Processing Session: {uploadSessionId ? uploadSessionId.slice(-8) : pollingSession.slice(-8)}
                  </div>
                  {sessionStatus ? (
                    <>
                      <div className="text-sm text-blue-600">
                        Status: <span className="font-medium">{sessionStatus.session_status}</span> |
                        Failed: <span className="font-medium text-red-600">{sessionStatus.failed_files}</span>
                        {sessionStatus.processing_files > 0 && (
                          <span> | Processing: <span className="font-medium text-amber-600">{sessionStatus.processing_files}</span></span>
                        )}
                        {sessionStatus.uploaded_files > 0 && (
                          <span> | Uploaded: <span className="font-medium">{sessionStatus.uploaded_files}</span></span>
                        )}
                      </div>
                      <div className="text-xs text-blue-500 mt-1">
                        Checking status every 15 seconds • {sessionStatus.documents?.length || 0} documents in session
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="text-xs text-blue-500 mt-1">
                        Checking status every 15 seconds • Session may still be starting up
                      </div>
                    </>
                  )}
                </div>
          ...
          <AlertDescription>
            <div className="flex items-center gap-3">
              ...
              <div className="font-medium text-green-800">
                Processing completed! You can now preview the extracted data and generate Excel files.
              </div>
            </div>
          </AlertDescription>
        </Alert>
      ...
      <div className="flex gap-4">
        <Button
          ...
          variant="outline"
        >
          {isProcessingComplete ? 'Preview' : 'Preview (Upload & Process First)'}
        </Button>

        <Button
          disabled={selectedFiles.length < 1 || !outputFormat || uploading || pollingSession !== null}
          onClick={handleSubmit}
          size="lg"
          className="px-8 py-3"
        >
          {isUploadingToS3 ? (uploadProgress || 'Uploading to Cloud...') :
           uploading ? 'Processing...' : 
           pollingSession ? 'Processing...' : 
      ...
      <PreviewModal
        ...
        outputFormat={outputFormats.find(f => f.format_id === outputFormat)?.format_name || outputFormat}
      />
    </div>
  )
}
